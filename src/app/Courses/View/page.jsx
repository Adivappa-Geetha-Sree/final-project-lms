...
export default function VeiwCourse() {
    ...
    let getCourses = () => {
        axios.get(`${apiBaseUrl}admin/course/view`)
            .then((res) => res.data)
            .then((finalres) => {
    ...
    let singDel = (id) => {
        axios.delete(`${apiBaseUrl}admin/course/delete/${id}`)
            .then((res) => {
                if (res.data.status) {
    ...
    let multiDel = () => {
        ...
        axios.post(`${apiBaseUrl}admin/course/muldelete/`, obj)
            .then((res) => {

            })
        toast.success("checked items deleted successfully")
        getCourses();
    }
        ...
        <>
            ...
            <div class="flex  bg-[#F5F7FF]">
                ...
                <div class=" w-[84%] relative px-[30px] py-[50px] h-[92vh] bg-[#F5F7FF]">
                    <h1 class="text-[25px] font-[500] mb-[10px]">Course Table</h1>
                    <div class="">
                                            ...
                                            course.map((value, index) => {
                                                return (
                                                    <tr key={index}>
                                                        ...
                                                        <td className="border border-gray-400 px-4 py-2">{value.course_Prize}</td>
                                                        <td className="border border-gray-400 px-4 py-2">{value.course_Duraction}</td>
                                                        <td className="border border-gray-400 px-4 py-2">{value.course_discrp}</td>
                                                        <td className="border border-gray-400 px-4 py-2">
                                                            <img className='w-16 h-16 rounded-md object-cover' src={apiBaseUrl + staticPath + value.course_img}
                                                                alt="image.jgp" />
                                                        </td>
